...
if ($period === 'day') {
    ...
    $cursor = $start;
    for ($i = 0; $i < 24; $i++) {
        $slotKeys[] = $cursor->format('Y-m-d H:00:00');
...
$filename = 'emsp-stats-' . $period . '-' . date('Ymd-His') . '.csv';
header('Content-Type: text/csv; charset=UTF-8');
header('Content-Disposition: attachment; filename="' . $filename . '"');
echo "\xEF\xBB\xBF";

$out = fopen('php://output', 'w');
fputcsv($out, ['Periode', 'Uploads', 'Approbations']);
foreach ($slotKeys as $idx => $key) {
    fputcsv($out, [
        $labels[$idx] ?? $key,
        (int) ($uploadsMap[$key] ?? 0),
        (int) ($approvalsMap[$key] ?? 0),
    ]);
}
fclose($out);
exit;
